...
inline unsigned char inportb_p(unsigned int port)		/* Input a byte from a port	*/
{
   unsigned char ret;
...
inline void outportb_p(unsigned int port,unsigned char value)	/* Output a byte to a port	*/
{
   asm volatile ("outb %%al,%%dx;outb %%al,$0x80"::"d" (port), "a" (value));
}

inline void outportw_p(unsigned int port,unsigned int value)	/* Output a word to a port	*/
{
